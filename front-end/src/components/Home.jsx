  ...
  const saveUpdate = async () => {
    ...
    try {
      const { data } = await axios.put(
        `/api/pro/products/${editingProduct._id}/quantity`,
        {
  ...
  return (
    <div className="p-4">
      <div className="flex flex-row items-center justify-between">
        ...
        <button
          className="h-10 w-16 bg-red-500 hover:bg-red-700 text-white mb-5 rounded-lg"
          ...
          {products.map((product) => (
            <div key={product._id} className="border p-4 rounded shadow">
              <div className="flex flex-row justify-between">
                ...
                <button
                  className="h-10 w-16 bg-emerald-500 hover:bg-emerald-700 text-white mb-5 rounded-lg"
